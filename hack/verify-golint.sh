...
KEYSTONE_ROOT=$(dirname "${BASH_SOURCE}")/..
source "${KEYSTONE_ROOT}/hack/lib/init.sh"

keystone::golang::verify_go_version
...
failure_file="${KEYSTONE_ROOT}/hack/.golint_failures"
...
all_packages=(
	$(go list -e ./... | egrep -v "/(third_party|vendor|staging/src/k8s.io/client-go/pkg|generated|clientset_generated)" | sed -e 's|^github.com/kubernetes-incubator/kube-arbitrator/||' -e "s|^_${KEYSTONE_ROOT}/\?||")
)
failing_packages=(
	$(cat $failure_file)
)
